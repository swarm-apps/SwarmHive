...
} from "../registry/tauri/lib/update-dialog-visibility";
...
describe("dialog visibility invariant", () => {
  // 核心不变量:任何 status × 任何 upgradeType 下,承载进度的弹窗至多一个。两个 modal Dialog
  // 同框时上层的 overlay 会吃掉下层的滚动与点击——这条不变量就是防它。
...
describe("forceDialogVisible", () => {
  // 线上回归(移动端 v0.7.16 → 0.7.17,prompt 类型;桌面同源同 bug):曾据 status 判断,
  // downloading 一到就弹出这个不可关的强制弹窗,冒充「需要更新」把用户锁到下载结束。
