// tauri-adapter —— UpdateAdapter 的 Tauri 平台实现(shadcn registry 源码,会被复制进用户项目)。
//
// 设计要点(见 add-registry-web-tauri/design.md):
// - check 走 @tauri-apps/plugin-updater 的 check()(内置 minisign 验签),**不用** SDK 的
//   checkUpdate(那是 RN 适配用的);从 update.rawJson.swarmhive 归一化成 SDK ReleaseInfo。
// - download / install **拆开**用 Update 实例(plugin-updater v2 支持独立 download/install)。
// - client_id 经 `X-Client-Id` header 传给 server,让灰度分桶在 server 端生效
//   (plugin-updater 运行时只能传 header、不能传自定义 query)。
//
// 依赖(本 registry item 的 npm dependencies,shadcn add 时自动安装):
//   @swarm-hive/sdk, @tauri-apps/plugin-updater, @tauri-apps/plugin-process, @tauri-apps/plugin-store
