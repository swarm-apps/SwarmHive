...
- **客户端 SDK 层**（docs/14 SDK 规划首次落地，架构修订为 1 npm + 2 registry + ports/adapter）：`add-update-sdk-core` ✅（`packages/sdk` headless 核心）→ `add-registry-web-tauri` ✅（tauriAdapter + useUpdate + 6 UI 组件，分发走 **GitHub raw**——`shadcn add` 是开发时操作、项目开源公开，**不做 server `/r` host**）→ `add-docs-website` ✅（已 apply 2026-06-04：Fumadocs 官网+文档站，dogfood mock live preview + iframe 隔离预览，GitHub Pages 子路径站 `swarm-apps.github.io/SwarmHive/`）。**RN Android 主线**（阶段7，Expo-first，调研拍板 2026-06-04）：`add-update-check-rn-android` 📝（server APK 端点，versionCode 整数闸门/ABI 匹配/`android_min_version_code` 强更，apply-ready，**阻塞项**）→ `add-sdk-android-check` ✅ 归档（`checkUpdateAndroid`+`normalizeAndroid`+`ReleaseInfo.kind?` 轻接缝）→ `add-registry-rn` 🚧 40/45（Expo-only，rnAdapter + 6 RN 组件方案 A 安装器 + config plugin;NativeWind+RNR 样式重构已 commit;剩 §10 模拟器/真机验证）；`add-ota-provider` 📝（Phase 2 占位，OTA=provider 扩展层，docs/11 两候选保持开放不预选）。让 SwarmDrop/SwarmNote 从第三方 ToolSetLink 迁到自托管 SwarmHive。
