...
```text
  ...
  通知层（横切，依赖 auth-and-rbac + mail-infrastructure + app-release-artifact）：
        ...
        └──────────────────────────────────────┘     剩: add-notification-im-providers(需 explore)
                                                      + delivery per-attempt 时间线
