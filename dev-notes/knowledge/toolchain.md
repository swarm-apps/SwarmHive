...
- **⚠️ tag 碰撞坑（已修，含一个二次坑）**：cargo-dist 的 `release.yml` 默认触发器是贪婪的 `**[0-9]+.[0-9]+.[0-9]+*`，它**也匹配 `sdk-v0.1.0`** —— 推 `sdk-v0.1.0` 会**同时**触发 release.yml 跑 cargo-dist，因 tag 映射不到 dist 包在 `plan` 步失败。**修法（已应用）**：把 release.yml 的 tag 模式手改成 `v[0-9]+.[0-9]+.[0-9]+*`（限 `v` 前缀，CLI 用 `v0.1.0`，SDK 的 `sdk-v*` 不再匹配）。GitHub glob 不支持负向匹配，没法 `tags-ignore`，只能这样限制。
  - **二次坑（实测踩到）**：cargo-dist 的 `dist host`/`dist plan` 启动时会**重新生成 release.yml 并与实际文件比对**，手改会让它报 `regenerated: @@ ... help: run 'dist init'` 并**以 exit 255 中止整个 release**（不是警告，是硬失败）。**必须在 `dist-workspace.toml` 的 `[dist]` 加 `allow-dirty = ["ci"]`**，cargo-dist 才会跳过 CI 文件一致性检查、容忍手改。代价：`dist generate` 不再自动更新 release.yml，升级 dist 后要手动重生成 + 重新应用 tag 限定。
