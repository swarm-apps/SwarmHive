...
- **⚠️ tag 碰撞坑（已修）**：cargo-dist 的 `release.yml` 默认触发器是贪婪的 `**[0-9]+.[0-9]+.[0-9]+*`，它**也匹配 `sdk-v0.1.0`** —— 实测推 `sdk-v0.1.0` 会**同时**触发 release.yml 跑 cargo-dist，因 tag 映射不到 dist 包在 `plan` 步**失败（红但无害，下游全跳过、不会误发 CLI）**。**修法（已应用）**：把 release.yml 的 tag 模式手改成 `v[0-9]+.[0-9]+.[0-9]+*`（限 `v` 前缀，CLI 用 `v0.1.0` 形式 tag，SDK 的 `sdk-v*` 不再匹配）。GitHub glob 不支持负向匹配，没法在保留 `tags` 的同时 `tags-ignore`，只能这样限制。**⚠️ `dist generate` 会把这行覆盖回贪婪默认 —— 重新生成后必须重新应用**（release.yml 该行有同样的提醒注释）。
